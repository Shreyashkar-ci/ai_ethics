...
# Modify the initial data loading part
...
if uploaded_file is not None:
    ...
    # Dynamically select columns for analysis
    st.sidebar.header("Fairness Analysis Configuration")
    
    # Select ground truth column
    ground_truth_col = st.sidebar.selectbox(
        "Select Ground Truth Column", 
        df.columns.tolist()
    )
    
    # Select predictions column
    predictions_col = st.sidebar.selectbox(
        "Select Predictions Column", 
        df.columns.tolist()
    )
    
    # Select sensitive features
    sensitive_feature_cols = st.sidebar.multiselect(
        "Select Sensitive Features", 
        df.select_dtypes(include=['object', 'category']).columns.tolist()
    )
    
    # Select statistical target feature
    statistical_target_feature = st.sidebar.selectbox(
        "Select Statistical Target Feature", 
        sensitive_feature_cols
    )
    
    # Add a button to trigger analysis
    if st.sidebar.button("Perform Fairness Analysis"):
        # Prepare data for analysis
        ground_truth = list(df[ground_truth_col])
        predictions = list(df[predictions_col])
        
        # Prepare sensitive features
        sensitive_features = df[sensitive_feature_cols]
        
        # Rest of the existing code remains the same, but use the dynamically selected columns
        fairness_metrics = FairnessMetrics(predictions, ground_truth)

        group_analyzer = GroupFairnessAnalyzer(
            sensitive_features,
            statistical_target_feature=statistical_target_feature
        )

        report = group_analyzer.generate_fairness_report(
            fairness_metrics,
            features_to_analyze=sensitive_feature_cols
        )
